...
interface ProjectNextStepsSectionProps {
  editing: boolean;
  title: string;
  description: string;
  type: QuickPlanType;
  ...
  onOpenAction: (action: ProjectNextAction) => void;
  getDestinationLabel: (task: TaskItem) => string;
}

const ProjectNextStepsSection: React.FC<ProjectNextStepsSectionProps> = ({
  editing,
  title,
  description,
  type,
  ...
  <>
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
        <Title level={5} style={{ fontSize: 14, margin: 0 }}>下一步</Title>
        <Tag bordered={false} style={{ margin: 0, fontSize: 10 }}>{tasks.length + actions.length} 项</Tag>
      </div>
      {!editing && <Button type="text" size="small" shape="circle" icon={<PlusOutlined />} title="新增计划" onClick={onOpenEditor} style={{ color: '#1677ff', background: '#edf7ff' }} />}
    </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 20 }}>
      {tasks.length === 0 && actions.length === 0 && !editing ? (
        <div style={{ padding: '18px 12px', textAlign: 'center' }}>
          <ClockCircleOutlined style={{ fontSize: 22, color: '#bfbfbf', marginBottom: 8 }} />
          <div><Text type="secondary" style={{ fontSize: 12 }}>暂无待办计划或行动建议</Text></div>
        </div>
      ) : (
        <>
          {tasks.length > 0 && actions.length > 0 && <Text type="secondary" style={{ fontSize: 10, fontWeight: 600 }}>计划任务</Text>}
          {tasks.map(task => {
            ...
            return (
              <div
                ...
                style={{
                  ...
                  background: checked ? '#f6ffed' : '#fafcff',
                  cursor: 'pointer',
                  transition: 'border-color 0.15s ease, background 0.15s ease, box-shadow 0.15s ease',
                }}
              >
                <Checkbox checked={checked} onClick={event => event.stopPropagation()} onChange={event => onToggleComplete(task, event.target.checked)} style={{ marginTop: 2 }} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <Text type="secondary" style={{ fontSize: 10, display: 'block', marginTop: 3 }}>点击进入{getDestinationLabel(task)}</Text>
                </div>
              </div>
            );
          })}

          {actions.length > 0 && <Text type="secondary" style={{ fontSize: 10, fontWeight: 600, marginTop: tasks.length ? 4 : 0 }}>系统建议</Text>}
          {actions.map(action => (
            <div
              key={action.id}
              role="button"
              ...
              style={{
                ...
                padding: '8px 10px',
                borderRadius: 7,
                border: `1px solid ${action.severity === 'high' ? '#ffccc7' : action.severity === 'medium' ? '#ffe58f' : '#e7edf4'}`,
                background: action.severity === 'high' ? '#fff1f0' : action.severity === 'medium' ? '#fffbe6' : '#fafcff',
                cursor: 'pointer',
              }}
            >
              <span style={{ marginTop: 2, flexShrink: 0, fontSize: 14 }}>
                {ACTION_KIND_ICON[action.kind] || <CheckCircleOutlined style={{ color: '#1677ff' }} />}
              </span>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <Text strong style={{ fontSize: 12, minWidth: 0 }} ellipsis={{ tooltip: action.title }}>{action.title}</Text>
                  {typeof action.count === 'number' && action.count > 1 && <Tag bordered={false} style={{ margin: 0, fontSize: 9 }}>{action.count}</Tag>}
                </div>
                <Text type="secondary" style={{ fontSize: 11, display: 'block', lineHeight: 1.4 }}>{action.detail}</Text>
              </div>
              <RightOutlined style={{ color: '#b8c1ce', fontSize: 10, marginTop: 4, flexShrink: 0 }} />
            </div>
          ))}
        </>
      )}
...
export default ProjectNextStepsSection;
